...
#transform 函数在load_data_fashion_mnist 函数中被定义，并传递给了DataLoader（本函数）
    ...
    def __len__(self):# //是整除 算出的长度是有多少个bath_size
...
def _get_batch(batch, ctx):#传入的batch 应该是一个batch_size的一组数据
    """return data and label on ctx"""
    if isinstance(batch, mx.io.DataBatch):
        data = batch.data[0]#? （train_data, test_data）（DataLoader类）中没有data 和label 因该是上面不满足条件
        label = batch.label[0]
    else:
        data, label = batch
    return (gluon.utils.split_and_load(data, ctx), #调用的是系统函数
            gluon.utils.split_and_load(label, ctx),
            data.shape[0]) #返回的因该是batch_size 
